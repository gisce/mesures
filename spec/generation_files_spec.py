...
from mesures.pmestqh import PMESTQH
...
class SampleData:
    ...
    @staticmethod
    def get_sample_data_pmestqh():
        basic_pmestqh = {
            "pm": "DK029141",
            "tipo_medida": 11,
            "timestamp": "2024-11-01 00:15:00",
            "season": 0,
            "method": 4,
            "ai": 10,
            "ae": 11,
        ...
        data_pmestqh = [basic_pmestqh.copy()]

        ts = "2024-11-01 00:15:00"
        for x in range(200):
            datas = basic_pmestqh.copy()
            ts = (datetime.strptime(ts, '%Y-%m-%d %H:%M:%S') + timedelta(minutes=15)).strftime('%Y-%m-%d %H:%M:%S')
            ai = randint(0, 5000)
            ae = randint(0, 2)
            r1 = randint(0, 30)
...
with description('An PMESTQH'):
    with it('is instance of PMESTQH Class'):
        data = SampleData().get_sample_data_pmestqh()
        f = PMESTQH(data)
        assert isinstance(f, PMESTQH)
    ...
    with it('gets expected content'):
        ...
        res = f.writer()
        # WARNING: timestamp is expressed as "yyyy/mm/dd HH" in file, but in dataframe is still ISO formatted
        expected = 'DK029141;11;2024-11-01 00:15:00;0;4;10;11;12;13;14;15'
        assert f.file[f.columns].to_csv(sep=';', header=None, index=False).split('\n')[0] == expected
